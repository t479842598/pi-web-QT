...
test("an orphaned paginated turn keeps its final answer outside process details", () => {
  const messages = [
    assistant("call", [toolCallBlock("c1", "read", { path: "a.ts" })]),
    toolResult("result", "c1"),
    assistant("answer", [textBlock("长正文\n".repeat(1000))]),
  ];
  const pipeline = buildHistoryPipeline(messages, ["call", "result", "answer"], "/tmp");
  const turn = pipeline.items[0];
  assert.equal(turn.userIdx, -1);
  assert.equal(turn.finalAssistantIdx, 2);
  assert.deepEqual(turn.finalAnswerMessage?.content, messages[2].content);
  assert.ok(turn.processBlocks.every((block) => block.type !== "text"));
});

test("a text-only orphaned prefix is an answer, not a collapsed thought", () => {
  const message = assistant("answer", [textBlock("visible answer")]);
  const turn = buildHistoryPipeline([message], ["answer"], undefined).items[0];
  assert.deepEqual(turn.finalAnswerMessage?.content, message.content);
  assert.equal(turn.processBlocks.length, 0);
});

test("a standalone shell execution at the start of history remains visible", () => {
  const message = { role: "bashExecution", command: "pwd", output: "/tmp", exitCode: 0 };
  const pipeline = buildHistoryPipeline([message], ["shell"], undefined);
  assert.deepEqual(pipeline.items, [{ kind: "single", idx: 0 }]);
});

test("an orphaned error without content keeps its visible provider error", () => {
  const errorMessage = { ...assistant("error", []), stopReason: "error", errorMessage: "Provider failed" };
  const turn = buildHistoryPipeline([errorMessage], ["error"], undefined).items[0];
  assert.equal(turn.finalAssistantIdx, 0);
  assert.equal(turn.finalAnswerMessage?.errorMessage, "Provider failed");
});

test("plain output inside process history is hoisted in order instead of labeled thinking", () => {
  const messages = [
    user("user", "work"),
    assistant("first", [textBlock("Progress report"), toolCallBlock("c1", "read", {})]),
    toolResult("result", "c1"),
    assistant("last", [textBlock("Final answer")]),
  ];
  const turn = buildHistoryPipeline(messages, ["user", "first", "result", "last"], undefined).items[0];
  assert.deepEqual(turn.processSegments.map((segment) => segment.kind), ["output", "group"]);
  assert.equal(turn.processSegments[0].blocks[0].text, "Progress report");
});
