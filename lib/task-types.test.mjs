...
test("keeps stable order on equal timestamps and sorts freshest first", () => {
  const tasks = [
    task(1, "queued"),
    task(2, "todo"),
    task(3, "canceled", { updatedAt: "2026-08-01T03:00:00Z" }),
    task(4, "done", { updatedAt: "2026-08-01T01:00:00Z" }),
    task(5, "done", { updatedAt: "2026-08-01T02:00:00Z" }),
  ];
  const grouped = groupTasksByColumn(tasks, true);
  // Equal updatedAt → stable input order preserved (the drag-order fallback).
  assert.deepEqual(grouped.todo.map((t) => t.id), [1, 2]);
  // Freshest updatedAt first within a column — the recently canceled row
  // (03:00) sits above the older done rows.
  assert.deepEqual(grouped.done.map((t) => t.id), [3, 5, 4]);
});
