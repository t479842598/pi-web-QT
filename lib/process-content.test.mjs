...
test("hoists text and image output without mixing it into thinking or tool details", async () => {
  const { splitProcessSegments } = await loadSubject();
  const origin = { phase: "process", placement: "standalone", sourceMessageIndex: 1 };
  const blocks = [
    { id: "thought", type: "thinking", thinking: "checking", origin },
    { id: "text", type: "text", text: "Long visible output\n".repeat(1000), origin },
    { id: "image", type: "image", source: { type: "url", url: "https://example.com/a.png" }, origin },
    toolCallBlock({ toolName: "read" }),
    toolCallBlock({ id: "agent", toolCallId: "agent" }),
    { id: "after", type: "text", text: "more output", origin },
  ];
  const segments = splitProcessSegments(blocks);
  assert.deepEqual(segments.map((segment) => segment.kind), ["group", "output", "group", "subagent", "output"]);
  assert.deepEqual(segments.flatMap((segment) => segment.blocks), blocks);
  assert.deepEqual(segments[1].blocks.map((block) => block.type), ["text", "image"]);
});

test("process and answer split agrees with message display for interleaved reasoning", async () => {
  const { splitAssistantContentBlocks } = await loadSubject();
  const message = assistant([
    { type: "text", text: "visible answer" },
    { type: "thinking", thinking: "late reasoning" },
    { type: "text", text: "answer end" },
  ]);
  const split = splitAssistantContentBlocks(message, { messageIndex: 4, entryId: "entry" });
  assert.deepEqual(split.resultBlocks.map((block) => block.type), ["text", "text"]);
  assert.deepEqual(split.resultBlocks.map((block) => block.origin.sourceBlockIndex), [0, 2]);
  assert.deepEqual(split.processBlocks.map((block) => block.type), ["thinking"]);
  assert.equal(split.processBlocks[0].origin.sourceBlockIndex, 1);
});
