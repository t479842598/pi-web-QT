...
test("does not resolve a parent path outside the default session storage", async (t) => {
  ...
  const originalListSessions = globalThis.__piListSessionsOverride ?? undefined;
  let scans = 0;
  globalThis.__piListSessionsOverride = async () => {
    scans += 1;
    return [];
  ...
  t.after(() => {
    ...
    if (originalListSessions === undefined) delete globalThis.__piListSessionsOverride;
    else globalThis.__piListSessionsOverride = originalListSessions;
...
test("a scan invalidated in flight retries before returning to its caller", async (t) => {  const originalListSessions = globalThis.__piListSessionsOverride ?? undefined;
  let scans = 0;
