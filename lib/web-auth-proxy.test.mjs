...
const { recordAuthSuccess } = await jiti.import("./auth-throttle.ts");
...
test("development and production both require auth even with a forged loopback Host", () => {
  ...
  try {
      ...
      for (const host of ["localhost:30141", "127.0.0.1:30141", "[::1]:30141"]) {
        for (const authorization of [undefined, `Basic ${Buffer.from("pi:wrong-fixture").toString("base64")}`]) {
          for (const [path, expected] of [["/api/sessions", 401], ["/sessions/view?tab=chat", 307]]) {
            // Each wrong guess feeds the shared throttle; clear it before every
            // assertion so the expected status reflects auth alone, not the
            // block accumulated by the loop's earlier iterations.
            recordAuthSuccess();
    ...
    // The loop above deliberately sends wrong passwords; clear the shared
    // globalThis throttle so later tests' valid Basic auth is not 429'd.
    recordAuthSuccess();
  }
});
