...
test("calculates padded base64 byte lengths and rejects malformed data", async () => {
  ...
  assert.equal(getBase64DecodedByteLength("YWJ="), null);
  assert.equal(getBase64DecodedByteLength("YR=="), null);
  assert.equal(getBase64DecodedByteLength("YQ==\n"), null);
  assert.equal(getBase64DecodedByteLength("not base64!"), null);
});

test("rejects invalid MIME, oversized, and too many image attachments", async () => {
  ...
  assert.match(validateAgentImages([{ ...image, type: "text" }]), /valid base64 image/);
  assert.match(validateAgentImages([{ ...image, mimeType: "image/" }]), /image MIME type/);
  assert.match(validateAgentImages([{ ...image, mimeType: "text/plain" }]), /image MIME type/);
