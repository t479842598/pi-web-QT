...
test("proxy never treats a loopback Host header as authentication", async () => {
  const source = await readFile(new URL("../proxy.ts", import.meta.url), "utf8");

  assert.doesNotMatch(source, /\bskipAuth\b|\bisLoopbackHost\b/, "Host is not proof of a local authenticated client");
  // The throttle-gated Basic branch still ends in credential validation; the
  // page-Basic path validates directly (v0.9.3 throttles only /api/* Basic).
  assert.match(source, /authenticated = isValidBasicAuthorization\(authorization, password\)/, "all environments validate credentials");
});
...
test("session DELETE reparents children atomically after shutting down their wrappers", async () => {
  ...
  // Scope to the re-parent loop: `readFileSync(childPath` also appears in the
  // subagent scan that runs earlier, so a whole-body indexOf would match that.
  const reparentLoop = body.slice(body.indexOf("for (const childPath of childPaths) {"));
  const shutdownIndex = reparentLoop.indexOf("getRpcSession(preview.id)?.shutdown()");
  const rereadIndex = reparentLoop.indexOf("readFileSync(childPath, \"utf8\")");
  assert.ok(
    shutdownIndex >= 0 && rereadIndex >= 0 && shutdownIndex < rereadIndex,
    "child wrapper shutdown precedes the file rewrite",
  );
  assert.match(body, /renameSync\(tmpPath, childPath\)/, "atomic tmp+rename rewrite");
  assert.match(body, /invalidateSessionManagerCache\(childPath\)/, "child cache invalidated");
  assert.match(body, /listAllSessions\(\{ force: true \}\)/, "cross-directory children discovered via session list");
  // Deleting a parent also deletes its subagent descendants.
  assert.match(body, /deletedSessionIds/, "descendant set is computed");
  assert.match(body, /abortSubagent\(deletedId\)/, "descendants are aborted before unlink");
  assert.match(body, /for \(const \[deletedId, deletedPath\] of deletedPaths\)/, "every descendant file is removed");
});
...
test("rpc-manager send() refuses commands on a dead wrapper and startup reuses no mid-shutdown wrapper", async () => {
  ...
  assert.match(source, /existing\.whenShutdown\(\)/, "waits out in-progress shutdown");
  assert.match(source, /Promise\.race\(\[/, "shutdown wait is bounded by a timeout");
...
test("proxy matches every path and scopes the static shortcut to development", async () => {
  ...
  assert.match(source, /NODE_ENV === "development"\s*&&\s*pathname\.startsWith\("\/_next\/static\/"\)/, "static shortcut dev-only");
  // The proxy covers every path, so an unauthenticated page load would redirect
  // the app's own bundle and white-screen the login page without this allowlist.
  assert.match(source, /isStaticAsset\(pathname\) \|\| pathname === "\/api\/web-auth"/, "static assets + login API bypass the redirect");
});
...
test("unreachable rollback path settles via waitForPromptSettlement instead of dropping the flag blindly", async () => {
  const source = await readFile(new URL("../hooks/useAgentSession.ts", import.meta.url), "utf8");
  const block = source.match(/const rollbackFailedQueueSend = useCallback[\s\S]*?\}, \[[^\]]*waitForPromptSettlement[^\]]*\]\);/);
  assert.ok(block, "rollback helper exists with settlement dep");
  assert.match(block[0], /serverConfirmedIdle/, "only a confirmed-idle server allows an immediate rollback");
  assert.match(block[0], /void waitForPromptSettlement\(sid\)/, "unreachable path polls to settlement");
});

test("queue and recovery setters dedupe by content", async () => {
  const source = await readFile(new URL("../hooks/useAgentSession.ts", import.meta.url), "utf8");
  assert.match(source, /sameQueuedMessages\(prev, next\) \? prev : next/, "queuedMessages setter dedupes");
  assert.match(source, /samePendingRecovery\(prev, next\) \? prev : next/, "pendingRecovery setter dedupes");
});
