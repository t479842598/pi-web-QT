...
test("negated or prefixed 'blocked:' text does not stop the goal", () => {
  // Regression: a bare includes("blocked:") matched "unblocked:" and
  // "not blocked:" and terminated a healthy goal.
  for (const text of [
    "Unblocked: the dependency is available again.",
    "I am not blocked: continuing now.",
    "The lock is unblocked: proceed.",
  ]) {
    const engine = new GoalEngine();
    engine.start("keep going");
    const { verdict, state } = engine.settleTurn(text, 10, 1);
    assert.equal(verdict.action, "continue", `"${text}" must not block`);
    assert.equal(state.status, "running");
  }
  // The explicit forms still work, wherever they appear.
  for (const text of ["blocked: need an API key", "Goal blocked by missing creds", "[goal: blocked]"]) {
    const engine = new GoalEngine();
    engine.start("keep going");
    assert.equal(engine.settleTurn(text, 10, 1).verdict.action, "blocked", `"${text}" must block`);
  }
});

test("prose mentioning completion does not end the goal", () => {
  // Regression: "once the goal is complete we can ship" contains
  // "goal is complete" as a substring and used to terminate the run.
  const engine = new GoalEngine();
  engine.start("ship it");
  const { verdict, state } = engine.settleTurn("Once the goal is complete we can ship, so next I will write the tests.", 10, 1);
  assert.equal(verdict.action, "continue");
  assert.equal(state.status, "running");

  // A marker at the start of a line or sentence still completes.
  for (const text of ["Goal complete.", "\n[goal: complete]", "Done. goal is complete"]) {
    const e2 = new GoalEngine();
    e2.start("ship it");
    assert.equal(e2.settleTurn(text, 10, 1).verdict.action, "complete", `"${text}" must complete`);
  }
});
