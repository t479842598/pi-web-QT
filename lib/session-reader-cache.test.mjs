...
test("cache evicts oldest entries beyond the cap", (t) => {
  const dir = isolate(t);
  reader.invalidateSessionManagerCache(); // clear for a clean count
  const files = [];
  for (let i = 0; i < 16; i++) {
    files.push(writeSessionFile(dir, `11111111-2222-4333-8444-5555555555${String(i).padStart(2, "0")}`));
  }
  for (const f of files) reader.openSessionCached(f);
  const cache = globalThis.__piSmCache;
  assert.ok(cache && cache.size <= 12, `cache bounded at 12, got ${cache ? cache.size : "no cache"}`);
});
