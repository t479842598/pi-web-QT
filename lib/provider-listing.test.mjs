...
test("keeps OpenCode Zen out of generic authentication lists", () => {
  const opencode = provider({ id: "opencode", name: "OpenCode Zen" });
  const opencodeGo = provider({ id: "opencode-go", name: "OpenCode Go" });
  assert.deepEqual(buildApiKeyProviderList([opencode]), []);
  assert.deepEqual(buildOAuthProviderList([opencode]), []);
  assert.deepEqual(buildApiKeyProviderList([opencodeGo]), []);
  assert.deepEqual(buildOAuthProviderList([opencodeGo]), []);
});
