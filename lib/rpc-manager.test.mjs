...
test("runtime sync swallows per-session failures and records redacted logs", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const syncSource = source.slice(
    source.indexOf("export async function syncOpenCodeZenRuntime"),
    source.indexOf("export function hasBusyRpcSessionForCwd"),
  );
  assert.match(syncSource, /Promise\.allSettled/);
  assert.match(syncSource, /recordErrorLog/);
  assert.match(syncSource, /removeRuntimeApiKey/);
  assert.match(syncSource, /opencode-zen-sync/);
});
