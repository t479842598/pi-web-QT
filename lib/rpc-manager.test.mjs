...
test("exact prompts are sent through before_agent_start instead of the SDK prompt state", async () => {
  ...
  // The exact prompt is applied ONLY through the before_agent_start extension;
  // the SDK prompt state is never mirrored or patched (runtime-verified by
  // rpc-manager-shutdown.test.mjs "an exact system prompt is reported by get_state
  // without touching the SDK prompt state").
