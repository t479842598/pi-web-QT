...
test("authorizes a bash log only when the exact session message references it", async () => {
  const { isBashOutputPathReferencedByEntries } = await loadSubject();
  const path = "/tmp/pi-bash-command_123.log";
  const entries = [{
    type: "message",
    id: "entry-1",
    parentId: null,
  ...
  assert.equal(isBashOutputPathReferencedByEntries(path, entries), true);
  assert.equal(isBashOutputPathReferencedByEntries("/tmp/pi-bash-command_123.log.bak", entries), false);
});
