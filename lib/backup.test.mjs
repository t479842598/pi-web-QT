...
test("adaptMcpConfig: url-only servers keep untouched, no empty command injected", () => {
  isolateAgentDir();
  const servers = {
    remote: { transport: "streamable-http", url: "https://example.com/mcp", lifecycle: "eager" },
  };
  const win = adaptMcpConfig(servers, new Map(), "win32");
  const s = win.servers[0];
  assert.equal(s.action, "keep");
  assert.equal(s.adapted, null);
  assert.equal(s.original.command, "");
});

test("restoreBackup: url-only server round-trips without command/args pollution", () => {
  const agentDir = isolateAgentDir();
  writeFileSync(join(agentDir, "mcp.json"), JSON.stringify({
    mcpServers: {
      remote: { transport: "streamable-http", url: "https://example.com/mcp", headers: { Authorization: "Bearer x" }, lifecycle: "eager" },
    },
  }));
  const zip = createBackupZip({ includeSecrets: true, includeSessions: false });
  const parsed = parseBackupZip(zip);
  try {
    restoreBackup(parsed, { categories: ["mcp"], skippedMcpServers: [] });
    const mcp = JSON.parse(readFileSync(join(agentDir, "mcp.json"), "utf8"));
    assert.equal("command" in mcp.mcpServers.remote, false);
    assert.equal("args" in mcp.mcpServers.remote, false);
    assert.equal(mcp.mcpServers.remote.url, "https://example.com/mcp");
    assert.equal(mcp.mcpServers.remote.transport, "streamable-http");
  } finally {
...
test("restoreBackup restores core, skills, local package with path rewrite, mcp adapt", () => {
  ...
  try {
    ...
    const mcp = JSON.parse(readFileSync(join(agentDir, "mcp.json"), "utf8"));
    const expectedWrapperCmd = process.platform === "win32"
      ? join(agentDir, "bin", "my-mcp.cmd")
      : join(agentDir, "bin", "my-mcp");
    assert.equal(mcp.mcpServers.wrapper.command, expectedWrapperCmd);
