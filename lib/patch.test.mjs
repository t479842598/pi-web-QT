...
test("parses zero-count additions without consuming following content", async () => {
  const { parseUnifiedPatch } = await loadSubject();
  const files = parseUnifiedPatch(`--- /dev/null
+++ b/created.txt
@@ -1,0 +1,1 @@
+created
`);
  assert.equal(files?.[0].rows.filter((row) => row.type === "line").length, 1);
  assert.deepEqual(files?.[0].rows.at(-1), {
    type: "line",
    left: { lineNo: null, text: "", type: "empty" },
    right: { lineNo: 1, text: "created", type: "added" },
  });
});
