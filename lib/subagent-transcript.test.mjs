...
test("readSubagentTranscript returns the full transcript by default (no limit)", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-subagent-full-"));
  try {
    const file = join(dir, "t.output");
    const total = 600; // beyond the old default
    const lines = [];
    for (let i = 0; i < total; i++) {
      lines.push(JSON.stringify({ type: "user", message: { role: "user", content: `msg ${i}` } }));
    }
    writeFileSync(file, lines.join("\n") + "\n");
    const parsed = readSubagentTranscript(file);
    assert.equal(parsed.length, total, "default limit returns all lines");
    assert.equal(parsed[0].text, "msg 0", "first line is the first message");
    assert.equal(parsed[total - 1].text, `msg ${total - 1}`, "last line is the newest");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
