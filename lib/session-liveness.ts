...
function scheduleLeaseExpiry(lease: LeaseRecord): void {
  ...
  // Never hold the process open for a lease timer.
  const unref = (lease.timer as unknown as { unref?: () => void }).unref;
  unref?.call(lease.timer);
}

/**
 * Keep one selected browser session alive while its lease keeps being renewed.
 *
 * The lease backs itself with a liveness provider, so rpc-manager's idle
 * eviction preserves the session for as long as the browser holds the lease.
 */
...
/** Renew every live browser lease for a session; expired leases are dropped. */
