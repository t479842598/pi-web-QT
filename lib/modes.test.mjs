...
test("chat modes map to the persisted axes and round-trip back", () => {
  for (const mode of CHAT_MODES) {
    const axes = chatModeAxes(mode);
    assert.equal(
      chatModeFromAxes(axes.collaborationMode, axes.toolApprovalMode),
      mode,
      `${mode} round-trips through its axes`,
    );
  }

  // The four ZCode semantics: plan is read-only-first, build asks, edit
  // auto-edits, yolo skips approvals.
  assert.deepEqual(chatModeAxes("plan"), { collaborationMode: "plan", toolApprovalMode: "ask" });
  assert.deepEqual(chatModeAxes("build"), { collaborationMode: "normal", toolApprovalMode: "ask" });
  assert.deepEqual(chatModeAxes("edit"), { collaborationMode: "normal", toolApprovalMode: "auto" });
  assert.deepEqual(chatModeAxes("yolo"), { collaborationMode: "normal", toolApprovalMode: "yolo" });

  // Any unknown pair falls back to the safest mode rather than undefined.
  assert.equal(chatModeFromAxes("normal", "ask"), "build");
});
