  ...
  writeOpenCodeZenConfig(
    {
      ...
      autoSwitch: true,
      cooldownMs: 60_000,
      externalAccess: { enabled: true, port: 7474, apiKey: "ext-key", freeModelsOnly: true },
    },
    "a2",
  );
...
// ─── 冷却/轮转状态持久化（opencode-zen-state.json）─────────────────────

function zenAccounts(ids) {
  return ids.map((id) => ({
    id,
    note: id,
    apiKey: `key-${id}`,
    enabled: true,
    proxy: { protocol: "http", enabled: false, url: "", port: 0, username: "", password: "" },
  }));
}

function zenConfig(accounts) {
  return {
    accounts,
    autoSwitch: true,
    cooldownMs: 60_000,
    externalAccess: { enabled: false, port: 7474, apiKey: "", freeModelsOnly: true },
  };
}

test("persists cooldown state and restores it after a runtime restart", async () => {
  const { writeOpenCodeZenConfig, createOpenCodeZenFetch } = await jiti.import("./opencode-zen.ts");
  // 全局态配置 3 个账号（写入隔离的 testAgentDir）
  writeOpenCodeZenConfig(zenConfig(zenAccounts(["a", "b", "c"])));

  // 第一轮：三个账号全部 429（日级冷却）→ 状态落盘
  const auths = [];
  const firstFetch = createOpenCodeZenFetch(async (_input, init) => {
    auths.push(new Headers(init?.headers).get("authorization"));
    return new Response("busy", { status: 429 });
  });
  const first = await firstFetch("https://opencode.ai/zen/v1/responses", {
    method: "POST",
    body: JSON.stringify({ model: "x", input: "hi" }),
  });
  assert.equal(first.status, 429);
  assert.deepEqual(auths, ["Bearer key-a", "Bearer key-b", "Bearer key-c"]);

  // state 文件已写入冷却
  const stateFile = join(process.env.PI_CODING_AGENT_DIR, "opencode-zen-state.json");
  assert.ok(existsSync(stateFile));
  const persisted = JSON.parse(readFileSync(stateFile, "utf8"));
  assert.deepEqual(Object.keys(persisted.cooldownUntil).sort(), ["a", "b", "c"]);
  assert.equal(persisted.cooldownUntil.a.daily, true);
  assert.ok(persisted.activeIndex >= 0);

  // 模拟进程重启：清掉全局态，新 fetch 从磁盘恢复冷却
  delete globalThis.__piOpenCodeZenState;
  let callsAfterRestart = 0;
  const secondFetch = createOpenCodeZenFetch(async () => {
    callsAfterRestart += 1;
    return new Response("busy", { status: 429 });
  });
  const second = await secondFetch("https://opencode.ai/zen/v1/responses", {
    method: "POST",
    body: JSON.stringify({ model: "x", input: "hi" }),
  });
  // 全池日级冷却 → 503，不再硬试任何已限额账号（若冷却丢失会再发请求拿 429）
  assert.equal(second.status, 503);
  assert.equal(callsAfterRestart, 0);
});

test("writeOpenCodeZenConfig keeps cooldown of surviving accounts and drops removed ones", async () => {
  const { writeOpenCodeZenConfig } = await jiti.import("./opencode-zen.ts");
  writeOpenCodeZenConfig(zenConfig(zenAccounts(["a", "b", "c"])));
  // 直接注入冷却：a/b 未过期，c 已过期
  const state = globalThis.__piOpenCodeZenState;
  const now = Date.now();
  state.cooldownUntil.set("a", { until: now + 60_000, daily: true });
  state.cooldownUntil.set("b", { until: now + 120_000, daily: true });
  state.cooldownUntil.set("c", { until: now - 1, daily: true });

  // 保存配置时移除账号 c：c 的冷却被清理，a/b 的冷却必须保留
  writeOpenCodeZenConfig(zenConfig(zenAccounts(["a", "b"])));
  assert.deepEqual([...state.cooldownUntil.keys()].sort(), ["a", "b"]);
  assert.equal(state.cooldownUntil.get("a").daily, true);
  assert.equal(state.cooldownUntil.get("b").until, now + 120_000);
});
