...
test("parses null-delimited Git status entries including renames", async () => {
  ...
  assert.deepEqual(entries, [
    { path: "components/App.tsx", indexStatus: " ", worktreeStatus: "M" },
    { path: "notes.txt", indexStatus: "?", worktreeStatus: "?" },
    { path: "src/new-name.ts", originalPath: "src/old-name.ts", indexStatus: "R", worktreeStatus: " " },
  ]);
});

test("classifies Git status for Quick Changes badges", async () => {
  const { classifyGitStatus } = await loadSubject();
  const classify = (pair) => classifyGitStatus({ path: "file.ts", indexStatus: pair[0], worktreeStatus: pair[1] });
