...
test("all project resource loaders enforce project trust", async () => {
  ...
  assert.match(rpcSource, /this\.syncProjectTrust\(\);\s*await this\.inner\.reload/g);
  ...
  assert.match(skillsRouteSource, /Project resources must be trusted before modifying project skills/);
  assert.match(skillsRouteSource, /isApiRequestAllowed\(req\)/);
});

test("the trust API invalidates models and destroys inactive runtimes", async () => {
  ...
  assert.match(source, /await destroyRpcSessionsForCwd\(result\.cwd\)/);
