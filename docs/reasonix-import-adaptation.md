...
> APPDATA 多根适配已合入正式版 v0.9.17 基线（对应 git commit 60467c2，qt/beta 分支）。
...
7. **2026-08-05 平铺布局支持** — Windows/CLI 布局 `~/.reasonix/sessions/*.jsonl` 此前不被识别（仅支持 mac 的 projects 布局），导致 UI 上 reasonix 选项 unavailable。`lib/import-sources.ts` 新增 `reasonixSessionsFlatDir()`、平铺发现（按文件名前缀分组）、统一 `reasonixProjectSessions()`；`parseReasonixFilename` 对非 mac 命名容错（回退文件 mtime，provider/modelId=unknown）；平铺项目 cwd 回退 homedir。

## APPDATA 多根适配（已合入 60467c2 / v0.9.17）
...
- `reasonixProjectSessions()` / `discoverReasonix()`：**遍历所有候选根目录**（projects 布局 + sessions 布局），跨根去重（`seen` 集合），按文件名前缀（第一个 `-` 前）分组。

适配后 `discover` 在 Windows 下实测 available=true（43 会话）。
