...
## 6. 2026-08-31 补充实施：标题任务与渐进加载

- **标题生成全局治理**：同一会话请求合并，进程内最多 2 个模型调用、最多 20 个排队项；支持 DELETE 取消，取消/超时后最多等待 5 秒回收底层模型 run，避免供应商忽略 abort 时长期占槽。
- **标题临时 wrapper 回收**：仅为冷会话标题生成创建的 AgentSessionWrapper，在生成结束且无订阅/无运行时立即 shutdown；正常聊天已存在的 wrapper 不受影响。
- **标题上下文内存边界**：最近消息优先，全局文本预算严格限制为 8,000 字符；最近 40 条无用户消息时仅补最近一个用户/compaction 来源，不再复制完整会话。
- **大会话渐进加载**：Web 首屏使用 `view=initial` 只返回最近 50 条 context；分支树、统计和活跃时长由 `/details` 后台加载并合并，切会话时 AbortController 取消陈旧请求。
- **详情单次聚合**：stats 与 totalActiveMs 合并为一次 entries 遍历；最近分支分页由反向扫描替代全 entries Map。
- **长工具回合 UI**：处理步骤模式改为标签页内独立持久化，旧 localStorage 时间线值不再跨窗口污染；运行时允许手动收起，minimap 按渲染后的 ProcessGroup 聚合标记。
