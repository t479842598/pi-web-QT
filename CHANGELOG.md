...
## v0.18.4 — 2026-09-13（热修：点会话标题弹出的全文卡片关不掉）

v0.18.3 让顶栏标题真正可读可点之后，暴露了点开后全文卡片**没有任何关闭途径**的老缺陷：遮罩没有点击处理、卡片没有关闭按钮，唯一出口是 Escape 键——手机上没有 Esc，点开后只能刷新页面。

**修复**：遮罩、卡片本体、卡片右上角 ✕ 三处任意点击都关闭（Escape 保留）。实测 390×844 四条路径（遮罩/卡片/✕/Esc）全部关闭。

## v0.18.3 — 2026-09-13（移动端标题/弹层/悬浮按钮修复；登录会话 Cookie 改 SameSite=Lax + 弹回原因诊断）

### 移动端三处显示修复

**标题占位符泄漏**：会话标题回退链把 pi 写进文件的字面量占位符 `(no messages)` 当标题直接渲染（顶栏被 320px 预留挤压后只剩 `(no …`）。新增 `lib/session-display-title.ts` 统一归一化（占位符/mode 注入块过滤 + name→firstMessage→id→fallback 链），替换 AppShell 顶栏、侧边栏项目面板、ProjectsPanel（2 处漏网）、AgentSessionPanel、SessionSearch、UsageConfig 共 9 处；空会话顶栏兜底显示项目名。

**底部统计弹层裁切**：`.is-session` 弹层此前只有 `right:0` + `max-width`，手机上统计按钮换行后弹层左缘跑出视口（session id 左半截消失）、三列并排把数字裁成 `83,07:`。现在移动端三个信息弹层改为视口钳制的 `position:fixed`（左右各 8px、贴锚点上方、可滚动、随旋转/缩放重算，`lib/popover-layout.ts` 纯函数 + 测试），三列统计窄屏竖排。

**悬浮按钮几何**：插队/排队面板改为**中线定位**（`top:0 + translateY(-50%)`），面板竖直中线精确落在输入框上边线上（实测 713 vs 712，±1px），按钮加高时自动跟随；「滚动到底部」胶囊改为**圆形箭头按钮**，经 ChatInput 新增的 `topCenterControl` 槽位渲染在输入框上边线正中（实测圆心 195 = shell 中心 195），与右侧插队面板横向相距 >77px 不再重叠。移动端隐藏顶栏「刷新会话」按钮。

**顶栏标题**：移动端中区从绝对定位居中改为流内左对齐（`flex:1` + 去掉 320px 预留），标题不再被右侧图标遮挡，仅超长时省略号 + 点按弹全文。移除移动端原生 `title` 提示（iOS 上糊且被顶栏裁切）。

### 登录会话保持（Safari 反复要求重输密码）

- **SameSite `Strict`→`Lax`**：Strict 会在任何跨站发起的导航（从聊天 App 点链接进入）下扣住 Cookie，30 天有效会话被当成未登录直接 307 到登录页——这是「登录后过段时间又要输密码」的最可能代码层根因。
- **弹回原因诊断**：proxy 307 到 `/login` 时带上 `reason=invalid`（有 Cookie 但校验失败=过期/改密）或 `reason=missing`（浏览器根本没回传 Cookie=无痕模式/清理了网站数据/Safari「防止跨站跟踪」「退出时清除历史记录」）。登录页按原因显示对应提示，下次再出现一眼定位，不用再全链路排查。
- 服务端 cookie 本身（httpOnly、30 天、HMAC 签名、Secure 随协议）经复查无误。

**验证**：1137 测试全绿、tsc 0 错、lint 0 error；构建产物 CSS 逐条抽查（`rm -rf .next` 全量重建，`chat-input-top-center`/`web-login-hint`/中线 transform 在包内、旧 `top:-22px` 已消失）；390×844 视口实测登录页两种提示、标题靠左、统计弹层全宽（left 8 / right 382 @390vw）、圆钮压线、插队面板中线 713≈上边线 712、子代理会话页标题正常。

## v0.18.2 — 2026-09-13（移动端样式审查修复；登录 cookie 保存自检）

### 移动端（≤640px / 触屏）逐页样式审查与修复

**弹层与溢出**：系统提示词弹层（`.is-system`）补上移动端视口钳制（390px 屏实测 374px，此前 520px 冲出右缘）；侧栏 5 处固定 320px 下拉改为 `min(320px, 88vw-16px)`；BranchNavigator 分支下拉按视口钳制 left/width；压缩错误提示窄屏钳宽换行（桌面保持单行）。

**触屏可用性**：代码块复制/预览浮层与用户消息动作组补类名，`@media (hover:none)` 下强制可见可点（此前触屏上永远点不到）；侧栏/消息操作/文件查看器状态栏小按钮在触屏手机上扩盒至 32px（触屏笔记本不受影响）。

**iOS 聚焦缩放**：设置弹窗、模型配置、侧栏、扩展输入、文件编辑器输入统一 16px（此前 12–14px 触发 Safari 聚焦放大）。

**布局**：FileViewer 状态栏（四处分支）改为可换行 + 路径省略号；composer 工具条窄屏换行；session-info-bar 窄屏换行、stats 按钮 token 芯片可折行；流式浮层按钮在触屏下上移不再压输入区；模型成本 4 列栅格手机端降为 2 列；登录页横屏可滚动；项目/worktree 控件 10px→11px。

**safe-area**：补上 `viewport-fit=cover`（此前所有 `env(safe-area-inset-*)` 规则在 iOS 上实际取 0，等于白写），standalone 模式标题栏加刘海避让。

### 登录状态自检

手机 Safari 登录成功后刷新仍要求重输密码的场景：服务端 cookie 链路实测正确（http 不发 Secure、https 发 Secure、30 天有效），问题在浏览器拒存/清除。登录页现在会在密码正确后校验 cookie 是否真的保存，未保存时明确提示「浏览器没有保存登录状态」，不再无声弹回登录页。

**验证**：tsc/lint 0 错、1124 测试全绿；390×844 与 320×568 视口 + 触屏模拟逐页实测；发版前经三遍独立审查（正确性/回归契约/完整性）并修复其全部发现。

## v0.18.1 — 2026-09-13（修登录页样式丢失与图标裂图；重做为居中布局）

### 修复：登录页完全没有样式（只显示裸 HTML）

v0.18.0 的**构建产物里不包含**登录页样式 —— `globals.css` 末尾的 `.web-login-*` 段（17 条选择器）整段缺失。比对构建 CSS 可确认：`file-tab-close`（源码 L2659）之前的选择器全部在，之后的整段都没有。

**原因**：那次构建复用了旧的 CSS 编译缓存，产物早于 `globals.css` 的最终写入。**影响**：0.18.0 的登录页在用户实际打开时是无样式的裸 HTML。

**修复**：清理 `.next` 后全量重建，样式已确认进入产物（`web-login-password` / `web-login-submit` 均在构建 CSS 中）。

### 修复：品牌图标显示为裂图

登录页原本用 `next/image`，其优化端点 `/_next/image?url=%2Ficon.png` 返回 **400**，导致图标加载失败（浏览器显示裂图占位）。

**修复**：改用普通 `<img>` 直接引用 `/favicon.svg`（实测 200 且为真实 SVG），不再依赖图片优化器 —— 裸服务器与隧道场景下也能稳定显示。

### 登录页按需求重做

- **密码输入框居中显示**，提交按钮位于输入框**下方**、与输入框同宽；整体在视口内水平垂直居中。
- **桌面与手机自适应**：手机端输入框/按钮高 50px、字号 16px（便于点按，并避免 iOS 聚焦时自动缩放），桌面 46px / 15px。
- **品牌改为本项目实际名称 `Pi Agent Web`**（此前写死 `Pi Web`），图标与标题/副标题纵向居中。
- 按钮与错误提示均走主题 token：禁用灰底灰字、启用 accent 底 + `var(--accent-fg)`（深色主题下为深色字，可读）；错误文案居中显示。

**实测**（生产构建，桌面 1440 与手机 390 各跑一遍）：图标 naturalWidth 256 正常、品牌名正确、输入框文字居中、按钮在输入框下方且同宽、整体居中；空密码禁用 / 有密码启用 / 错误提示「密码不正确」/ 成功登录跳转 `/` 全部正确；浅色与深色两个主题配色均取自 token。

## v0.18.0 — 2026-09-12（对齐上游 v0.9.1；移除自建 wss 中继；模式与授权弹窗对齐 ZCode）

本次为功能性版本，含**破坏性变更**：自建 wss 中继已整体移除（代码、公网服务器部署与本机配置），远程接入只保留 Cloudflare Tunnel 直连。

### 子代理：移植上游 v0.9.1 的 P0 能力

此前同一会话最多跑 4 个子代理，**超出的直接报错**；现在改为排队。

- **并发排队**：新增 `lib/subagent-queue.ts`（每父会话独立 FIFO + 槽位），`queued` 状态随会话文件持久化；新增「并发子代理数」设置（默认 10、上限 32，位于设置 → 子代理）。
- **worktree 隔离**：profile 可设 `isolation: worktree`，子代理在独立 `pi-web-agent-<uuid>` 工作树运行，写入不落在父工作区；完成/中止/启动失败三条路径都会清理，清理失败会把原因记入元数据。
- **profile 兼容**：支持 `promptMode: replace|append`（替换系统提示词）、`skills`/`extensions` 的布尔或白名单写法、`color`/`isolation`/`persist_session`。保存 profile 时**保留本应用不管理的前置字段**。
- **恢复会话**：`Agent` 工具新增 `resume` 参数，可继续一个已持久化的子代理会话。
- **扩展工具选择器**：`tools:` 支持 `ext:<name>` / `ext:<name>/<tool>` / `ext:<name>/*` / `ext:*`。
- **删除父会话时连带删除其子代理**：此前子代理转录会残留成挂在已删根上的孤儿。

### 修复

- **SSE 重连后 shell 输出不再丢失**：在途 tool 事件会被缓存并在重连时回放，`bash`/`powershell` 的流式输出在断线重连或中途打开会话时仍可见。
- **首条消息即可分叉**：此前只有产生过助手回复的会话能分叉。
- **会话模型显示与还原更准确**：模型名优先取显式的 `model_change` 并按当前分支解析；重开会话会恢复保存的模型。
- **大 JSON 响应启用 gzip**（≥1KB），会话列表与详情加载更快。
- **大文本预览分页**：超过 256KB 的文件不再直接 413，改为按 `?offset=` 分段加载，查看器提供「加载更多」；截断时不再自动切到 Markdown/HTML 预览（渲染半个文档会误导）。
- 扩展提示词按 Markdown 渲染；@ 文件选择器限制在输入框上方的可见区域内且键盘选择可循环；支持 `?sidebar=collapsed` 嵌入启动；打开活跃会话时保留流式输出；内置命令防重复提交；富文本粘贴保留链接。

### 模型额度：换用 provider 用量，移除本地 DeepSeek 余额专栈

- 模型设置里可查询 **8 家** provider 的官方额度：openai-codex / deepseek / openrouter / moonshotai（含 cn）/ minimax（含 cn）/ vercel-ai-gateway。凭据取自 pi 已保存的认证信息，只允许官方端点（自定义 baseUrl 会被拒绝）。
- 相应移除本地那套 DeepSeek 钱包余额（`/api/deepseek/balance`、余额 chip）。
- **保留**：CNY 计价与 token 用量统计（与「额度」是两件事）。

### 新增：浏览器密码登录

设置 `PI_WEB_PASSWORD` 后，浏览器访问页面会跳转到**应用内登录页**（30 天免登录），不再弹浏览器的原生凭据框；设置页可退出登录。**修改密码会立刻作废所有已登录设备**。

> **Basic 认证仍在全部路由有效**（页面与 API），因此桌面壳、移动端与内存看门狗**无需任何改动**即可继续使用。

### 新增：选中会话保活

选中的会话即使空闲也不会被回收（90 秒租约 + 前端每 30 秒续租）；租约意外丢失时会自动重建连接。

### 模式选择器与手动授权弹窗：对齐 ZCode

- **模式选择器**改为 ZCode 的四个模式（图标与语义一致）：**计划** / **变更前确认** / **自动编辑** / **完全访问**。勾选一个即同时决定协作方式与审批档位；pi-web 特有的「目标」保留为第五项。
- **手动授权弹窗**重做为 ZCode 的样式：四个**带序号**的选项（1 允许 / 2 始终允许 / 3 拒绝 / 4 始终拒绝），键盘 `1`-`4` 直接应答、方向键与 Tab 移动、回车确认、`Esc` 关闭。「始终允许/拒绝」会真正记住规则（只记住该具体命令/文件，而非整个工具）。

### 移除：自建 wss 中继

- 删除 `relay-server/` 子项目、`docs/relay-deployment.md`、`lib/relay-client.ts`、`lib/relay-runtime.ts`、`lib/relay-http-bridge.ts`、`app/api/pair`、配对 UI 与全部 `pair.*` 文案。
- 公网服务器上停止并禁用 `pi-web-relay`、移除 nginx vhost、删除部署目录/环境变量/证书与 systemd 单元。
- 本机 launchd 移除 `PI_WEB_RELAY_*` 环境变量。
- **远程接入仍可用**：Cloudflare Tunnel 直连方式不受影响。

### 样式审查

修复深色模式下的可读性问题：18 处 accent 底上的硬编码白字改为 `var(--accent-fg)`（深色主题的强调色是浅色，白字不可读）；模型测试结果 chip、备份提示、文件上传状态点等 light-only 硬编码色改为主题 token；`QueueRecoveryDialog` 补上 `Esc` 关闭。新增守卫测试防止这类硬编码回归。

## v0.17.15 — 2026-09-12（中继页不再显示注入的提示词；思考只留一行摘要；工具行改为单行）

### 修复：对话里把系统注入的提示词也显示了

pi 把系统提示块（`<delivery-profile>…`、`<goal-profile>…`、旧的 "You are in PLAN MODE." 标题块）**作为 user 消息的一部分**注入，而中继页把所有 user 消息都当对话气泡渲染 —— 于是满屏都是「Prioritize a verified, complete result over minimizing model calls…」这类给 agent 的指令，真正说过的话反而被淹没。

现在按 pi-web 自己的规则（`lib/modes.ts` 的 `stripModeInstructionBlocks`，中继页是独立静态包无法 import，故移植同一套正则）在渲染前剥离这些块。

**实测**：`delivery-profile|economy-profile|goal-profile` 全部不再出现，用户气泡只剩「看一下官方有没有什么更新…」「继续」这类真实输入。

### 修复：思考展开了整段内部独白

ZCode 对每个思考块只显示**一行摘要**（「思考 · 持续了几秒」），点开才看正文。中继页此前默认 `open`，把整段推理铺满屏幕 —— 这是「太乱」的主要来源。

现在默认收起，摘要带时长。pi 不记录思考块的耗时，故用与上一条消息的时间差推算（ZCode 显示的也是一个概数）。

**实测**：16 个思考块全部收起，摘要形如「思考 · 1 秒」。

### 工具行改为单行摘要

- 摘要行显示**动词 + 目标**（如「运行 ssh -o BatchMode=yes root@…」），而不是把整个参数对象 JSON 铺开；动词按工具映射为「运行 / 查阅 / 编辑 / 搜索 / 抓取 / 子代理」，对齐 ZCode 的「查阅 / 编辑」标签。
- 失败的调用在摘要行直接标「失败」，不必展开。
- 展开后显示命令本体与结果，收敛在卡片内。

### 验证

注入块不再泄漏、16 个思考块全部收起、18 个工具行均为单行摘要且无空行；中继 49 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。

## v0.17.14 — 2026-09-12（会话页补齐 Markdown 渲染与消息操作条；修空会话标题；顶栏/composer 按截图重排）

### 修复：消息内容是纯文本，没有渲染 Markdown

ZCode 的画面里消息是**渲染后的 Markdown**（编号列表、带底色的行内代码、加粗），而中继页把整段当纯文本显示，「`biz_code=14`」这类行内代码完全看不出层次。

- 引入 `marked`（pi-web 已依赖的同一个库，取其 UMD 构建作为中继页静态资源）。
- 消息体经**白名单净化**后注入：只保留 Markdown 相关标签，链接限 `http(s)` 并加 `relnoopener`，其余属性（`onclick`/`style` 等）一律剥离。消息是模型输出，而该页与中继同源，不能直接 `innerHTML`。
- 补齐 Markdown 样式：段落/标题/列表/有序列表标记、行内代码底色与圆角、代码块卡片、引用左边线、表格边框、分隔线。

**实测**：一个真实会话渲染出 22 个段落、6 个列表（15 项）、**90 处行内代码**、8 个代码块、51 处加粗。

### 新增：消息操作条

ZCode 在每条消息下方有复制/赞/踩/分叉与时间戳。中继页补上**复制**与**时间戳**；赞/踩/分叉需要中继未暴露的接口，故不显示（宁可少也不放不能用的按钮）。

### 修复：空会话标题显示成 `(no messages)`

pi 把字面量 `(no messages)` 写进空会话的 `firstMessage`，而标题直接取了该字段。现在识别这个占位符，空会话显示「新会话」。

### 顶栏与 composer 按截图重排

- 会话头从 3 个按钮收敛为 **2 个**（`...` + 侧栏），与 ZCode 一致；「切换显示模式」与「打开完整界面」移入 `...` 菜单。
- composer 改为**单盒结构**：输入框在上、控制行在下（模型选择 + 发送），不再是「模型下拉占一整行 + 蓝色方块按钮」。
- 发送按钮圆角 10px、28×28。

### 验证

完整点击巡检（39 个项目卡、`...` 菜单开关、侧栏 inert 开合、composer 结构、发消息 18 秒完成）全部通过、无 console 错误。中继 49 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。

### 仍未做

附件上传、`@` 提及文件、斜杠命令、赞/踩/分叉、长会话虚拟化与分页、Bot Channel。

## v0.17.13 — 2026-09-12（中继页会话页按 ZCode 实测值对齐；修工具结果不渲染、流式过程不可见、输入法误发）

### 中继页会话页：按 ZCode 实测值重做（34 项全部对齐）

沿用首页那套方法：用 Playwright 打开真实的 ZCode 远程会话页逐元素读计算样式。这次发现的不只是数值偏差，**有三处是结构相反**：

| 项 | ZCode 实测 | 修正前 |
|---|---|---|
| **用户消息** | **左对齐**、占满宽度、圆角 `12px 2px 12px 12px`、bg `rgba(255,255,255,.05)`、padding `12px 16px` | 右对齐气泡、`max-width:85%` |
| **助手消息** | **无气泡无边框无圆角**，纯文本 | 有背景+边框的 12px 圆角气泡 |
| **思考块（收起）** | **裸文字**，无背景无边框 | 彩色左边框卡片 |
| 思考块（展开） | 仅 1px 左边线 + 14px 缩进 + 240px 上限滚动 | 卡片 |
| 工具摘要行 | **无边框无圆角**普通行 | 一律卡片 |
| 工具展开内容 | 12px 卡片、16px 内边距 | 同 |
| 角色标签 | **没有**「你/助手」 | 有 |
| 顶栏 | **两层**：44px 导航 + 48px 会话头（共 92px） | 单层 49px |
| 导航栏底色 | `#202020` | — |
| 消息区 | padding `56px 16px 20px`、间距 **20px** | 12px 间距 |
| composer 输入框 | 圆角 **16px**、边框 `rgba(255,255,255,.15)`、min 40 / max 160px | 圆角 8px |
| composer 按钮 | **28×28**、圆角 8px | 36px |

新增 token `--color-header` / `--color-surface` / `--color-input-border`（含浅色与显式主题两套）。会话页补上主题菜单（此前只有首页有）。

### 修复：工具结果从不渲染

`toolResult` 消息被直接 `continue` 跳过，所以中继页只显示工具名、看不到任何输出。现在按 `toolCallId` 建立索引，把结果渲染进工具展开卡；`isError` 用红色边框与「结果 · 失败」标注。

### 修复：流式期间看不到思考与工具

之前只增量更新正文，思考与工具要等整轮结束重新加载才出现。现在从 `message_update` 增量提取 thinking / toolCall 块实时渲染。

### 修复：中文输入法回车会误发消息

Enter 发送没有判断 `isComposing`——**打中文时按回车确认候选词会直接把消息发出去**。现在 `isComposing`（含 `keyCode === 229`）时不发送。

### 验证

- 与 ZCode 实测值逐项比对：**34 项全部一致**（自动化脚本输出「不一致项: 0」）。
- 三个缺陷实测：IME 回车消息数 25→25（未误发）；发一条会调用 bash 的消息，**2 秒时**就出现 18 个思考块与 23 个工具行（证明流式可见）；最终渲染 40 个工具结果卡，模型据此正确读出 `hello`。
- 中继 49 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。测试会话已清理。

### 本次未做（明确列出，避免再次出现"宣称已对齐"）

附件上传、`@` 提及文件、斜杠命令、markdown/代码块渲染、重新生成/分叉、长会话虚拟化与分页、Bot Channel。

## v0.17.12 — 2026-09-12（配置移出安装目录；中继页按 ZCode 实测值对齐；新增会话创建；修发送按钮卡住）

### 中继页：按 ZCode 远程页的**实测值**对齐（不再靠推断）

用 Playwright 打开真实的 ZCode 远程页，逐元素读取计算样式，再回头对照本实现。修正的偏差：

| 项 | ZCode 实测 | 修正前 |
|---|---|---|
| 卡片圆角 | 8px | 12px |
| 卡片外边距 | 8px（下） | 12px |
| 图标按钮 | 24×24，圆角 6px | 44px（触屏）/ 36px |
| 顶栏高度 | 49px（0 垂直内边距） | 62px |
| 顶栏标题 | 16px/500 | 14px/500 |
| 副标题 | 14px/400，上间距 4px | 13px/500，2px |
| 会话行最小高 | 48px（8px/10px 内边距 → 62px） | 52px |
| 会话行悬停 | `rgba(255,255,255,0.1)` | 0.04（过淡） |
| 类型徽标 | 10px/400，`2px 6px`，浅底色 | 11px/500，无底色 |

触屏的点击区不再靠放大按钮实现（那会破坏对齐），改为**视觉尺寸不变、用透明伪元素扩大热区**。

### 新增：中继页新建会话

ZCode 把「新建」放在**每个工作区卡上**（`onStartDraftInWorkspace`），此前本实现遗漏了这个入口。现在每个项目卡右上角有 `+` 按钮，展开的项目列表末尾也有一行「新建会话」；调用 `/api/agent/new` 的 `ensure_session` 创建空会话后直接进入聊天页。

（卡片因此从 `<button>` 改为 `role="button"` 的 `div`——按钮不能嵌套按钮。）

### 修复：发送后按钮一直停在「停止」

两个原因叠加：

- `await stream.catch(...)` 等的是 SSE 长连接，服务端在运行结束后仍可能保持连接，于是 `finally` 永不执行；
- `finally` 里先 `await loadContext(...)`（又一次隧道往返），把「复位按钮」挡在了后面。

改为：不 await 事件流；复位移到任何 await 之前；并增加服务端兜底——轮询 `/api/agent/running`，即使终态事件丢失也能结束等待。

### 新增：配置移出安装目录（`npm i -g` 不再丢配置）

`.env` 放在包目录里，每次 `npm install -g` 都会被整目录替换掉，密码和主机白名单静默丢失（本次开发中反复踩到）。

- 启动器改为从 `~/.pi/agent/pi-web.env`（或 `~/.pi/pi-web.env`）读取配置，**在解析命令行参数之前**加载，因此文件里的端口/主机同样生效。
- 环境变量优先于文件，仍可由 supervisor 覆盖单项。
- 日志只打印生效的**键名**，不打印值。
- 已把现有配置迁移到 `~/.pi/agent/pi-web.env`（0600）。

### 验证

用 Playwright 对本机 30141 实测三种宽度：1440px / 1024px 显示模型名（220px）与思考级别（58px），390px 为纯图标（28/24px）。中继页完整巡检验证：39 个项目卡、搜索过滤、收起/展开全部、按项目/时间线双模式（608 行）、主题三态切换、新建会话、两种消息渲染模式、侧栏滑入（343px，主区 inert）、发消息收发正常。中继 49 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。

## v0.17.11 — 2026-09-12（中继 token 持久化修好「链接失效」；中继页补齐图标与动画；桌面端恢复模型/思考/模式文字）

### 修复：配对链接重启即失效（v0.17.10 的「永久链接」实际是假的）

中继此前把配对 token **只存在内存**里，任何一次重启（包括每次部署）都会清空。后果有两个，且第二个更隐蔽：

- 用户保存的链接在下次部署后变成废纸；
- `~/.pi/agent/web-pairing.json` 里仍留着那条 token，`create` 又把它复用返回，于是页面永远报「配对信息无效」，从现象上完全看不出根因。

改动：
- 新增 `relay-server/src/token-store.mjs`：token 以**哈希**形式原子落盘到 `tokens.json`（0600），只保留未过期的记录；损坏文件按空库处理，不会拖垮服务。
- 中继启动时恢复各设备的 token（日志会打 `restored tokens`）。
- **桌面侧在复用已保存的链接前先向中继确认它还在**（比对 tokenHash）；不一致就重新签发，避免继续分发死链接。
- 中继的 `pair_list` 增加 `tokenHash` 字段（哈希不是凭证，原始 token 从不离开桌面端）。

**实测**：签发链接 → 重启中继 → 同一条链接仍可配对成功（重启日志 `restored tokens count:1`）。

### 修复：首页折叠一个项目后，点另一个会把前一个又展开

`applyDefaultExpansion`（首次访问展开最近 2 个项目）此前用「展开集合是否为空」判断是否已初始化。于是**收起所有项目**时集合变空，下一次点击触发重绘，默认展开逻辑又把最近两个项目加了回来。

改为用一次性的 `defaultsApplied` 标志；「展开全部 / 收起全部」也会置位该标志，因为那是用户的明确选择。

**实测**（逐项验证）：收起全部 → 只点开一个 → 确只展开它 → 再点另一个 → 第一个保持展开 → 收起第一个 → 第二个不受影响。

### 中继页：图标与动画对齐 ZCode

- 图标从 Unicode 符号（`⇈ ⌄ › ⟳ ⤢ ← ✕ ⇅`）换成 **ZCode 同款 Lucide 图标**（以 `<symbol>` 精灵内联，形状来自 lucide-react 的真实路径）：chevron-right、chevrons-down-up、chevrons-up-down、rotate-cw、arrow-left、panel-right、maximize、x、search、sliders 等。
- 补动画与过渡：卡片箭头**旋转**（而非换字符）表示展开、展开列表淡入上移、按下时 `:active` 反馈（触屏没有 hover）、统一的 150ms ease-out 过渡；并遵守 `prefers-reduced-motion`。
- 顶栏按 ZCode 的构成收敛为「标题 + 主题菜单」：新增**主题切换**（浅色 / 深色 / 跟随系统，持久化，跟随系统时监听系统变化）；「打开完整界面」与「刷新」移到区块头部。
- 会话页新增侧栏入口按钮；侧栏打开时主聊天区置 `inert`，与 ZCode 一致。

### 修复：桌面端把模型名、思考强度、协作模式都显示成了纯图标

上一轮「移动端图标化」改过了头——三个按钮被固定成 24–28px 纯图标，**桌面端也一样**，丢失了模型名与思考强度文字。

现在按视口区分：桌面显示「图标 + 名称」（模型名超长省略、上限 220px），手机才收成纯图标。

**实测**：1440px / 1024px 显示模型名（宽 220px）与思考级别（宽 58px）；390px 为纯图标（28px / 24px）。

### 验证

中继新增 6 项 token 持久化测试（往返、缺失、损坏、畸形记录、不含原始 token、空设备省略）。中继 49 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。

## v0.17.10 — 2026-09-11（配对改为永久可复用链接，支持多设备）

### 问题

远程配对链接是「10 分钟 + 一次性」的扫码语义，导致：

- 链接生成稍晚点开就提示「二维码已过期」；
- 同一条链接**不能被第二台设备使用**——兑换即作废，多设备场景完全不可用；
- 会话凭据 30 天到期后需要重新扫码。
...
- 中继的 token 记录新增 `reusable` 语义。可复用 token 不写 `usedAt`，因此同一链接可在任意多台设备反复打开；`ttlMs: 0` 表示永不过期。
- 由永久链接签发的会话凭据**也不设过期**——访问权限通过「吊销链接」收回，而不是等手机自己过期。
- 吊销链接会连带断开**所有**由它进入的设备（此前只影响单个会话）。

**配对面板改为「一条固定链接」**（设置 → 远程配对）：

- 打开即显示链接全文，一键复制（并可直接选中，剪贴板不可用时也能手动复制）。
- 链接持久化在 `~/.pi/agent/web-pairing.json`，重启后仍是同一条，不会每次打开都生成新的。
- 新增「换一条」（重建并吊销旧链接，避免旧链接作为第二把永久钥匙残留）与「吊销」（收回全部访问）。
- 显示当前在线设备数与中继连接状态。
- 保留「一次性二维码」接口（`action: "qr"`）供临时借用他人手机的场景，不再是默认路径。

### 安全说明

永久 + 可复用意味着**这条链接等价于一枚长期钥匙，谁拿到谁就能进**。这是为多设备使用做的取舍，配套的兜底是：面板可随时吊销（吊销后所有设备立即断开），并显示在线设备数便于发现异常连接。

### 验证

- 中继新增 5 项测试：同一链接连续配 3 台设备各获得独立凭据、连接数统计、吊销断开全部设备并作废凭据、永久链接的凭据无过期、一次性 token 过期仍报「已过期」而非「已使用」。
- 中继 43 项 + 主项目 1079 项测试通过，`tsc` 0 错，`lint` 0 error。

## v0.17.9 — 2026-09-11（中继页功能对齐 ZCode 中继端；HTTP 隧道让手机可用完整界面；新增轨迹/折叠双显示模式）

### 中继页：功能对齐 ZCode 远程端

对照 ZCode 客户端内嵌的 `mobileHome` / `mobileShell` 实现逐项对齐：

- **项目分组 + 逐个展收**：展开状态用 Set 记忆并持久化；首次访问默认展开最近活跃的 2 个项目（ZCode 默认全展开，但本机 600+ 会话全展会淹没首屏）。
- **全展/全收按钮**：与 ZCode 一致，仅在「按项目」分组时出现（时间线模式无展开概念）。
- **整理双模式**：按项目 / 按时间线；**排序**：更新时间 / 创建时间。两者收进一个 `⇅` 菜单，对应 ZCode 的单一整理控件。
- **会话页**：右侧滑入侧栏浮层（含遮罩，宽度 `min(88vw,28rem)`）、重连提示 toast、加载遮罩——对齐 `mobileShell` 的浮层与状态反馈。
- **触控**：图标按钮 44px、会话行 52px、菜单项 44px（触屏环境），会话行含运行中转圈与状态徽标。
- **响应式网格**：项目卡手机单列 → 768px 两列 → 1024px 三列；展开的会话列表跨整行，避免撑破相邻卡片。
- 新增搜索框（本机 600+ 会话必需；ZCode 无此项）。
- Bot Channel 留禁用态说明入口（ZCode 有微信/飞书/Lark/Telegram 四通道，pi-web 尚未实现该后端集成）。

### 新增：HTTP-over-WS 隧道（手机可用完整界面）

中继原先只转发 WebSocket 帧，手机拿不到会话与模型数据。

- 手机的所有 HTTP 请求经隧道转到本机 pi-web：请求/响应分片流式传输，**SSE 边读边转发**（聊天流式依赖它）。
- 响应按 `cid` 精确回发给发起请求的那台手机，不再广播——一台设备可服务多台手机。
- `/web/*` 代理完整 pi-web 界面：HTML 里的 `/_next/`、`/api/`、`/icon` 及任意根相对路径（含重定向 `Location`）都正确加前缀，否则页面能打开但资源全 404。
- 安全：目标固定 `127.0.0.1`（不接受绝对 URL，无 SSRF）；仅配对连接可用；请求/响应分片与并发数设上限；未配对访问 `/web/` 返回 401。

### 新增：会话凭据（解决「刷新即失效」）

配对 token 是一次性的，刷新页面或打开 `/web/` 都会因 token 已用而被拒。

- 配对成功时签发 30 天会话凭据，存 localStorage + Cookie（`/web/` 的普通 HTTP 请求只能靠 Cookie 携带）。
- 重连优先用凭据；吊销 token 会连带作废由它派生的全部凭据。

### 新增：轨迹 / 折叠双显示模式

聊天页可切换两种消息渲染，设置持久化：

- **ZCode 轨迹**：按时间线串联的彩色条目——用户（蓝）、助手（青）、推理（紫）、工具调用、工具结果，配色取自 ZCode 的 `--color-trajectory-*`。
- **pi-web 折叠**：过程组折叠（推理 + 工具调用收进可展开面板），最终回答留在外面——沿用网页端的观感。

### 修复

- **聊天流式从未生效**：此前把 `POST /api/agent/[id]`（`type:"prompt"`）的响应体当流读取，但该接口只是接受命令并立即返回；真正的输出在 `GET /api/agent/[id]/events`（SSE）。现在先订阅事件流再发命令，避免丢失开头的增量。
- **会话与模型列表渲染为空**：`/context` 的 `messages` 嵌在 `context` 下而非顶层；`/api/models` 的模型数组是 `modelList`（`models` 是键值对象），`defaultModel` 是 `{provider, modelId}` 对象。
- **页面初始三视图同时可见**：HTML 里 `view-chat` / `view-status` 缺 `hidden`，首帧会短暂叠加显示。
- **宝塔 nginx 缓存导致部署后仍是旧页面**：面板全局启用了 `proxy_cache`（`proxy.conf`），而站点的 `location /` 未关闭缓存，HTML/JS 被缓存一小时。已加 `proxy_cache off` 并写入部署文档。

### 验证

- 三档视口实测（手机 390 / 横屏 844 / 桌面 1280）：无水平溢出；网格列数 1 / 2 / 3；图标按钮在触屏环境 44px。
- 真实数据实测：39 个项目 / 608 个会话；搜索「pi-web」→ 4 行；时间线模式 608 行；会话页消息 23 条、模型选择器 85 项、折叠模式 20 个过程组、轨迹模式 48 个条目。
- 中继 38 项测试（新增隧道、`/web` 前缀重写、会话凭据）、主项目 1079 项测试、`tsc` 0 错。

## v0.17.8 — 2026-09-11（新增自建 wss 中继与扫码配对；移动端尺寸 token；修复子代理行与工具结果的一批回归）

### 新增：第二条远程接入通道（自建 wss 中继 + 扫码配对）

- **为什么**：此前远程访问只有 Cloudflare Tunnel 直连一条路，要求本机可被手机直达，且每次都要手输密码。新通道让 pi-web **主动外连**自建中继（不暴露任何入站端口），手机访问中继托管的移动页，经 WebSocket 双向转发；配对改为**一次性 token 二维码**，扫码即免密接入。
- 中继是 `relay-server/` 子项目（Node + `ws`，仅此一个依赖），协议透明转发、不解析聊天内容；控制帧由中继应答，并带 `rid` 支持请求-响应。
- 配对 token **一次性**、默认 10 分钟有效；「已使用」与「已吊销」分开记录，设备列表能区分二者，吊销后立即踢掉对应连接。
- pi-web 侧新增「设置 → 远程配对」分区：生成二维码、复制链接、查看配对列表。
- **配对以中继为唯一 token 权威**：pi-web 不再本地存储 token，而是通过既有长连接代理 `pair_register` / `pair_revoke` / `pair_list`，避免两套实现语义漂移（见 `.lrnev/scenes/06-web-mobile/decisions/adr/0001-relay-owns-pairing-tokens.md`）。
- 部署：`systemd` 常驻 + Nginx/Caddy 反代（WS `Upgrade` 透传 + 关缓冲）+ HTTPS，见 `docs/relay-deployment.md`。家用规模 1 核 1GB 即可；中继与 pi-web 同机建议 1 核 2GB。
- 两个地址必须分开配置：`PI_WEB_RELAY_URL` 是 **WebSocket** 端点（桌面端出站），`PI_WEB_RELAY_PUBLIC_URL` 是 **HTTPS** origin（二维码链接）。写错会生成打不开的二维码。
- 中继鉴权加固：新增 `RELAY_DEVICE_SECRET`，公网部署必填——否则任何人都能冒充设备 ID、顶掉真实桌面端并自行签发配对 token。

### 移动端样式：引入尺寸 token 层

- `app/globals.css` 新增结构量 token：`--size-base` 及由其派生的 `--text-ui-xs…xl`、`--radius-xs…xl`、`--page-margin-mobile/desktop`。颜色仍由主题控制，尺寸不随主题变化。
- 移动侧栏宽度由固定 `280px / 85vw` 改为 `min(88vw, 28rem)`，宽屏手机上获得更舒适的阅读宽度。
- 新增 `docs/web-mobile-style-reference.md`，记录对齐基线（含 ZCode 远程端使用的容器查询断点），后续 composer 适配按此推进。

### 修复

- **后台子代理的流式更新会抹掉真实工具结果**：`tool_execution_update` 的合并方向写反了（`{...真实, ...临时}`），而后台运行的 `Agent` 在真实结果落地后仍会继续推送更新，于是 `content` 被清空。现在改为真实结果优先，临时更新只补 `details`，且仅在该行尚无结果时生效。
- **子代理派发失败后行不可点击、错误原因丢失**：此前仅凭 `toolName === "Agent"` 就提升为独立行，而失败分支返回的是 `details: undefined` + 错误文本，导致渲染成一行点不开的「死行」。现在只有运行中或携带合法 details 的调用才提升，失败回落普通工具组，错误原文照常可见。
- **非法状态被渲染成绿色「已完成」**：子代理 details 的类型守卫只校验了两个字段，`status` 任意值都会静默兜底成 completed。现在按白名单校验状态。
- **历史消息里每个子代理行各发一次 `/api/sessions/<id>`**：面板关闭时权威记录缺失，每行都会补拉且 id 互异、缓存无法命中。现在只对仍显示运行中的行补拉，并加 `AbortController`。
- **子代理行可能永远停在「运行中」**：Agents 面板关闭后其会话快照不再刷新，而该快照优先于行自身的补拉。现在关闭面板即清空快照。

### 测试

- 新增 20 项中继协议测试（设备注册、协议版本、双向转发、token 单次使用/过期/吊销、双设备不串线、请求-响应、设备密钥、设备数上限、重连踢旧客户端）。
- 补充子代理分段与守卫的边界用例（派发失败回落、非法状态、空输入）。
- 全量 1079 项测试通过；`tsc --noEmit` 0 错；`lint` 0 error。
