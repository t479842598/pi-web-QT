...
## v0.18.10 — 2026-09-25（补：手机端也去掉重复工具栏）

v0.18.9 只在桌面隐藏了上游那条重复的聊天区工具栏，移动端仍然保留，因此用手机远程连接时标题栏下依旧多出一行。本次把它在所有视口隐藏。

该行的按钮全部有替代入口，隐藏不丢功能：侧边栏与文件面板开关在标题栏，完整历史 / 系统提示词 / 分支切换 / token 统计在底部会话信息栏，生成标题在侧边栏会话操作里，子代理族谱在右侧面板。

## v0.18.9 — 2026-09-25（修复合并引入的三处 UI 回归）

v0.18.7 合并上游 v0.9.2/v0.9.3 时，上游对同批组件的重构与 fork 既有实现按文件取一侧，产生三处界面问题。

### 右侧文件面板「子代理」按钮换行、显示不全

上游「隐藏文件面板」按钮覆盖了 fork 的 Agents 族谱开关：按钮已是 36px 方形图标尺寸，却仍渲染「子代理」文字，文字被挤成换行；同时该按钮的点击只关闭面板，**Agents 族谱面板因此失去入口**。现拆成两个独立按钮，族谱开关恢复文字按钮尺寸，隐藏按钮改用面板图标。

### 标题栏下的重复工具栏

上游新增的聊天区顶部工具栏（完整历史 / 生成标题 / 系统 / 工具 / token 统计）与桌面标题栏、底部会话信息栏重复。生成标题等已在侧边栏有入口，故桌面隐藏该行，移动端保留。同时修正两处连带问题：该行内的浮层显式限定为移动端（`display:none` 会连带隐藏 `position:fixed` 后代），顶部浮层的定位改用独立锚点，不再依赖会随该行塌陷的引用。

### 复制按钮移入后不可见

复制浮层原以绝对定位锚在整条消息容器顶部，长轮次（数千像素）时该顶部早已滚出视口，在正文中段悬停自然看不到按钮（悬停事件本身一直正常）。改为粘性定位贴住可见区顶部。虚拟列表行的 `overflow: hidden` 会建立滚动容器、囚禁粘性后代，一并改为 `overflow: clip`。

### 验证

tsc 0 错、`node --test` 1645 通过 0 失败、lint 的 `react-hooks/rules-of-hooks` 违规 0；开发与生产构建均经浏览器实测三项修复，并复验子代理两项 UI（主对话运行中子代理状态行、转录面板历史轮次折叠）。
...
- 白名单补齐受信任 origin 的空根路径，连接页可正常加载；不放宽主机、端口、凭据或查询校验。
- 远程内容已开始加载后仍不能跳回壳内管理页面，原有管理 IPC 隔离保持。
- 回归覆盖 macOS 裸根/带斜杠根、Windows 根路径及远程回跳拦截；完整保留 v0.18.5 的正文、虚拟列表、会话恢复和权限修复。

## v0.18.5 — 2026-09-14（正文与思考分离、虚拟列表叠字修复；会话恢复与桌面权限加固）

### 聊天显示与输入恢复

- **正文不再误归“思考”**：普通文本和图片独立显示；分页窗口缺少用户消息时仍保留最终答案。晚到的 thinking 块不再吞掉已有正文，并补齐图片输出、空正文错误和小地图分段定位。
- **虚拟列表不再依赖切换会话恢复高度**：按已提交的行身份局部测量，后台内容展开通过 DOM 变化补测，图片加载和返回前台再次校准。虚拟区不再使用第二层 content-visibility 占位；修正初次加载及顶部扩展组件的坐标，保留离屏行高缓存，避免全表清缓存造成回弹。
- **失败输入与图片恢复**：草稿图片校验适配自身数据格式；明确拒收时撤回乐观消息并恢复文本、图片，保留用户后来输入的内容。网络结果不确定时仍对账，不盲目重复发送。

### 会话、任务与子代理可靠性

- 恢复队列在真实消息落盘前保留持久化副本；失败、停止或缺少落盘证据的条目留给用户恢复，不自动重放。
- 删除会话与在途冷启动建立逐会话屏障，阻止迟到初始化“复活”会话；重挂接子会话前等待已接收消息落盘。状态轮询不再延后 Stop 的回收期限。
- 子代理取消覆盖初始化、排队和模型预检阶段；终态及清理错误先落盘，再关闭会话，后台子代理仍独立运行。
- 任务合并等待真实终态并校验执行代际，由引擎将任务提交合入明确的目标分支；失败保留工作成果。只有验证成功且允许清理时才非强制移除工作树；无存活会话的合并同样支持取消与状态对账。
- 目标自动续跑包含当前目标正文，编辑目标后使用新内容。
- 原生移动端为创建、发送、事件订阅和异步回包增加会话代际保护，快速切换不会被旧响应接管，队列失败只移除自己的占位消息。

### 认证与桌面连接边界

- 配置密码后，开发环境也必须通过有效认证，不再仅凭 Host 为环回地址跳过密码。
- 桌面连接管理移至独立的受信壳内窗口，远程内容不再获得服务器配置、密码和服务启停权限。
- 桌面代理在注入 Basic 凭据前校验原始来源，固定绑定上游 origin；跨 origin 编辑不继承旧密码，重新连接使用新端口，旧入口不转发新目标。
- **兼容性说明**：桌面本地代理采用 Basic-only，双向移除 Cookie/Set-Cookie，不支持额外依赖 Cookie 登录的上游网关；浏览器直接访问 Pi Web 的 Cookie 登录不受影响。同源、明确证据下的省端口 Origin 兼容保留。
- 补齐 Tauri 自动更新产物生成开关，并同步桌面 npm/Rust 锁文件版本，发布流水线生成签名更新包及更新清单。

**验证**：1345 项 Node 测试通过；37 项 Rust 测试通过、2 项真实环境测试未运行；31 项 Flutter 会话回归及随机顺序复跑通过；TypeScript/Dart 检查通过，lint 零错误。独立生产 React 夹具复现并验证后台展开叠字修复；完整生产包与本地更新按本次发布流程验证。
