...
abstract final class AppleColors {
  // ── Accent palette (MonkeyCode-aligned, user-selectable) ──────────
  static const accentGreen = Color(0xff16b364);
  static const accentBlue = Color(0xff3d83f5);
  static const accentPurple = Color(0xff8b6cf0);
  static const accentOrange = Color(0xfff29a35);
  /// Legacy pi accent (teal), kept as a compat option.
  static const accentClassic = Color(0xff0d9488);

  /// Default accent — fresh green, matching MonkeyCode's default.
  static const accent = accentGreen;

  // ── Light palette (MonkeyCode-aligned: warm grey ground) ──────────
  static const ground = Color(0xfff3f3f0);
  static const surface = Color(0xffecece7);
  static const card = Color(0xffffffff);
  static const text = Color(0xff1d1d1a);
  static const textSecondary = Color(0xff76766f);
  static const textTertiary = Color(0xffa8a89f);
  static const faint = Color(0xffe0e0db);
  static const hairline = Color(0x0f141410);

  // ── Dark palette (MonkeyCode-aligned: near-black) ─────────────────
  static const darkGround = Color(0xff15161a);
  static const darkSurface = Color(0xff1f2025);
  static const darkSurfaceRaised = Color(0xff2b2c31);
  static const darkText = Color(0xffedeeec);
  static const darkSecondary = Color(0xff9fa0a0);
  static const darkHairline = Color(0x12ffffff);
}
...
/// Maps an accent name (or hex string) to its [Color]. Falls back to the
/// default fresh green for unknown values.
Color accentByName(String name) {
  return switch (name) {
    'green' => AppleColors.accentGreen,
    'blue' => AppleColors.accentBlue,
    'purple' => AppleColors.accentPurple,
    'orange' => AppleColors.accentOrange,
    'classic' => AppleColors.accentClassic,
    _ => colorFromHex(name) ?? AppleColors.accentGreen,
  };
}

/// The list of user-selectable accents (name → label key). `classic` keeps
/// the legacy teal so existing users can restore their old look.
const appleAccentChoices = <(String, String)>[
  ('green', 'accent.green'),
  ('blue', 'accent.blue'),
  ('purple', 'accent.purple'),
  ('orange', 'accent.orange'),
  ('classic', 'accent.classic'),
];

ThemeData buildAppleTheme(
  Brightness brightness, {
  Color accent = AppleColors.accent,
}) {
...
class AppleGlass extends StatelessWidget {
  ...
  /// When true, skips the BackdropFilter and uses a near-opaque fill instead
  /// (Android's blur is weak — this keeps panels readable at all times).
  final bool solid;
  ...
  Widget build(BuildContext context) {
    ...
    final useSolid = solid || reduceTransparency;
    return ClipRRect(
      borderRadius: borderRadius,
      child: useSolid
          ? DecoratedBox(
              decoration: BoxDecoration(
                color: color.withValues(alpha: dark ? .96 : .92),
                borderRadius: borderRadius,
                border: Border.all(
          ...
          : BackdropFilter(
              ...
              child: DecoratedBox(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: dark
                        ? AppleColors.darkHairline
                        : AppleColors.hairline,
                  ),
                ),
                child: Padding(
...
class PiWebColors extends ThemeExtension<PiWebColors> {
  ...
  /// User-selectable accent (MonkeyCode-style). Drives primary actions,
  /// selected states and chat bubble tints.
  final Color accent;

  /// Accent at ~12% alpha — chat bubble / chip background tint.
  Color get accentGhost => accent.withValues(alpha: .12);

  /// Accent at ~32% alpha — bubble border / focus line.
  Color get accentLine => accent.withValues(alpha: .32);
